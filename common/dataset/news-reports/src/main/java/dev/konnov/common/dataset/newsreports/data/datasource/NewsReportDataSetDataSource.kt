package dev.konnov.common.dataset.newsreports.data.datasource
...
class NewsReportDataSetDataSource @Inject constructor() : DataSetDataSource<String, NewsReport> {
    ...
    override fun get(size: Int): List<NewsReport> {
        ...
        for (i in 1..size) {
            dataList.add(generateNewsReport(i))
        }
        return dataList
    }

    /**
     * Title values to test CRUD operations on
     */
    override val oldParameterToUpdate: String
        get() = "Strange things"

    override val objectToInsertAsUpdate: NewsReport
        get() = NewsReport("not strange", "text replacer")

    override val parameterToDelete: String
        get() = "Nothing is lost"

    override val parameterToLoadBy: String
